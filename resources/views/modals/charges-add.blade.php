<div id="charges-add" class="fixed inset-0 bg-gray-800 bg-opacity-50 flex items-center justify-center hidden">
    <div class="bg-white shadow-lg rounded-lg max-w-md w-full p-4">

        <div class="flex justify-between items-center">
            <h2 class="text-2xl font-semibold">Add Charges to {{$account->student->first_name}}</h2>
        ...
        <form hx-post="{{ route('charges.add', $account->id) }}" hx-trigger="submit" hx-target="#charges-list-{{$account->id}}" hx-swap="innerHTML">
            ...
            <div class="flex justify-end mt-4">
                ...
                <button type="button" onclick="document.getElementById('charges-add').classList.add('hidden')" class="ml-1 py-2 px-2 bg-gray-500 hover:bg-gray-800 text-gray-100 rounded">Close</button>
            </div>
        </form>
