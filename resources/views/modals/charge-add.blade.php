<div id="charges-id-{{$account->id}}" class="fixed inset-0 bg-gray-800 bg-opacity-50 flex items-center justify-center">
    <div class="bg-white shadow-lg rounded-lg max-w-md w-full p-4">
        <div class="flex justify-between items-center">
            ...
            <button onclick="document.getElementById('charges-id-{{$account->id}}').classList.add('hidden')" class="text-gray-500 hover:text-gray-700">&times;</button>
        </div>
        <form
        ...
        hx-trigger="submit" hx-swap="outerHTML" hx-target="#charges-id-{{$account->id}}">
            ...
            <div class="flex justify-end mt-4">
                ...
                <button type="button" onclick="document.getElementById('charges-id-{{$account->id}}').classList.add('hidden')" class="ml-1 py-2 px-2 bg-gray-500 hover:bg-gray-800 text-gray-100 rounded">Close</button>
            </div>
        </form>
    </div>
</div>
