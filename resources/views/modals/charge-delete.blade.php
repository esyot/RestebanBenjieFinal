<div id="charge-delete-{{$charge->id}}" class="fixed inset-0 bg-gray-800 bg-opacity-50 flex items-center justify-center hidden">
    <div class="bg-white shadow-lg rounded-lg max-w-md w-full p-4">
        ...
        <div class="flex justify-between items-center">
            ...
            <button onclick="document.getElementById('charge-delete-{{$charge->id}}').classList.add('hidden')" class="text-gray-500 hover-text-gray-700">&times;</button>
        </div>
            <div class="mt-4">
            ...
            <div class="flex justify-end mt-4">
                <button onclick="document.getElementById('charge-delete-{{$charge->id}}').classList.add('hidden')" hx-get="{{ route('charge.delete', ['id'=>$charge->id]) }}" hx-trigger="click" hx-swap="outerHTML" hx-target="#charge-{{$charge->id}}" 
